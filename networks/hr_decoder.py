...
class DepthDecoder_task(nn.Module):
    def __init__(self, ch_enc=[64, 128, 216, 288, 288], scales=range(4), num_ch_enc=[64, 64, 128, 256, 512],
        ...
        # feature fusion
        self.convs["f4"] = Attention_Module(self.ch_enc[4] * 2, num_ch_enc[4])
        self.convs["f3"] = Attention_Module(self.ch_enc[3] * 2, num_ch_enc[3])
    ...
    def forward(self, input_features, depth_fea):
        outputs = {}
        feat = {}
        feat[4] = self.convs["f4"](torch.cat([input_features[4], depth_fea[-1]], 1))
        feat[3] = self.convs["f3"](torch.cat([input_features[3], depth_fea[-2]], 1))
...
class DepthDecoder(nn.Module):
    ...
    def forward(self, input_features,depth_fea,k):
        outputs = {}
        feat={}
        feat[4] = self.convs["f4"](torch.cat([input_features[4],depth_fea[-1]],1))
        feat[3] = self.convs["f3"](torch.cat([input_features[3],depth_fea[-2]],1))
        ...
        x = self.convs["X_04_Conv_0"](x)
        x = self.convs["X_04_Conv_1"](upsample(x))
        outputs[("disp", k,0)] = self.sigmoid(self.convs["dispconv0"](x))
        outputs[("disp", k,1)] = self.sigmoid(self.convs["dispconv1"](features["X_04"]))
        outputs[("disp", k,2)] = self.sigmoid(self.convs["dispconv2"](features["X_13"]))
        outputs[("disp", k,3)] = self.sigmoid(self.convs["dispconv3"](features["X_22"]))
        return outputs


class DepthDecoder_y(nn.Module):
    def __init__(self, ch_enc=[64, 128, 216, 288, 288], scales=range(4), num_ch_enc=[64, 64, 128, 256, 512],
                 num_output_channels=1):
        super(DepthDecoder_y, self).__init__()
        self.num_output_channels = num_output_channels
        self.num_ch_enc = num_ch_enc
        self.ch_enc = ch_enc
        self.scales = scales
